...
def create_table():
    connection=psycopg2.connect(database="mydatabase",user="hrishikesh",password="redhat",port=5432,host="localhost")
...
def add_employee(id,name,sal):
    connection = psycopg2.connect(database="mydatabase", user="hrishikesh", password="redhat", port=5432,
                                  host="localhost")
    cursor=connection.cursor()
